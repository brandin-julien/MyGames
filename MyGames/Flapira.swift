...
class Flapira: SKScene, SKPhysicsContactDelegate{
    
    let coinSound = SKAction.playSoundFileNamed("audio.mp3", waitForCompletion: false)
    ...
    override func didMove(to view: SKView) {
        ...
        let delay = SKAction.wait(forDuration: TimeInterval(1.5)) // Interval for spawn
...
//        birdTexture2.filteringMode = .nearest
        ...
        let birdTexture1 = SKTexture(imageNamed: "pira-01")
        birdTexture1.filteringMode = .nearest
        let birdTexture2 = SKTexture(imageNamed: "pira-02")
        birdTexture2.filteringMode = .nearest
...
//        let flap = SKAction.repeatForever(anim)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if moving.speed > 0 {
            if ( contact.bodyA.categoryBitMask & scoreCategory ) == scoreCategory || ( contact.bodyB.categoryBitMask & scoreCategory ) == scoreCategory {
                ...
                flashScreen(color: SKColor(red: 0, green: 1, blue: 0, alpha: 1), canRestart: false)
    ...
    func flashScreen(color : SKColor, canRestart: Bool){
